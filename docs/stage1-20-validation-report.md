...
2026-09-06 收到的两个压缩包合计构成 30 题候选池：

- 服务机器人仿真-能工智人队-刘轶凡.zip：20 题；20 个 XML 与 test.list 均和仓库最终题集逐文件 SHA-256 一致，21/21 匹配。
- 题目及跑分记录.zip：另含 10 题试跑包、命令记录和运行日志。

30 题整体静态校验结果为 0 error、0 warning，结构化指令和自然语言均无跨题重复。新增 10 题随后在当前冻结代码及同一官方平台上补跑官方客户端和 Iron，IT/NT 各一遍，共 40 次；所有运行均生成完整结果且无超时。

| 新候选 | 官方 IT | Iron IT | 官方 NT | Iron NT | Iron 两模式最低 | 最小相对优势 |
|---|---:|---:|---:|---:|---:|---:|
| 01 | 100 | 148 | 98 | 148 | 148 | +48 |
| 02 | 98 | 76 | 98 | 110 | 76 | -22 |
| 03 | 98 | 114 | 98 | 186 | 114 | +16 |
| 04 | 158 | 68 | 158 | 32 | 32 | -126 |
| 05 | 98 | 138 | 98 | 68 | 68 | -30 |
| 06 | 138 | 34 | 138 | 70 | 34 | -104 |
| 07 | 98 | 244 | 98 | 12 | 12 | -86 |
| 08 | 258 | 179 | 258 | 133 | 133 | -125 |
| 09 | 98 | 74 | 98 | 106 | 74 | -24 |
| 10 | 198 | 204 | 198 | 202 | 202 | +4 |

新增 10 题中，最高的两模式最低分为 202；原 20 题按两轮复跑取最低值后的最低分为 412。因此统一按稳定最低分排序时，前 20 名全部来自原题集，不需要替换。

回传记录曾显示新增 10 题得分为 318 至 401，但记录所用提交 ecb757a09757ada044832fec5240e40cb62ffeae 不存在于当前仓库对象库，无法核对其源码，且当时没有运行官方客户端基线。为保证比较口径一致，本次筛选采用当前冻结代码的重新实测结果，不使用该组不可追溯分数参与排名。
