...
void RDFW::MustChooseOne(void){
    ...
    if(stage==2){
        ...
        for(int i=0;i<tasks.size();i++){
             ...
             if(tasks[flag].behave!="open"||tasks[flag].behave!="close") GetSmallObjectStatus(tasks[flag].X[0]->id);
    ...
    */
    ...
    LOG(GREEN "Final-GOTO: choose hub=%d\n" RESET,chosen_loc);
        ...
        if (move_set.empty()) {
            ...
            // 实际搬运 ≤ 10 件
            const int MULTI_GOTO_LIMIT = 11;
            int moved = 0;
            for (unsigned id : move_set) {
                if (moved >= MULTI_GOTO_LIMIT) break;
                auto s = std::dynamic_pointer_cast<SmallObject>(objects[id]);
                if (!s) continue;
                if (s->location == chosen_loc) continue;
                if (location != s->location) {
                    if (!Move(s->location)) continue;
                }
                if (!HoldSmallObject(id)) continue;

                if (location != chosen_loc) {
                    if (!Move(chosen_loc)) { PutDown(id); continue; }
                }
                if (putdown_cons[id][chosen_loc] == 0) {
                    if (PutDown(id)) {
                        ++moved;
                        LOG(GREEN "Final-GOTO: moved obj[%u] to hub=%d (%d/%d)\n" RESET,
...
void RDFW::OptimizeMemoryUsage() {
 ...
 void RDFW::ApplyMustNearConstraintCorrection(){
    ...
    // 如果hold或plate在组内，则更新机器人位置
     if(hold_mustnear) {
        location = hold->location;
        plate->location = hold->location;
     }
     else if(plate_mustnear) {
        location = plate->location;
        hold->location = plate->location;
     }
